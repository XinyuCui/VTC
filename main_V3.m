...
monte_carlo_average_delay_of_hybrid=zeros(monte_carlo_iteration,1);
...
monte_carlo_average_packet_loss_of_hybrid=zeros(monte_carlo_iteration,1);
...
monte_carlo_average_packet_loss_of_hybrid_of_hop_level=zeros(number_of_node,monte_carlo_iteration);
...
for monte_carlo_index=1:1:monte_carlo_iteration
    ...
    cache_state_of_hybrid=zeros(number_of_request,number_of_node,number_of_video);    
    ...
    for request_index=1:1:number_of_request
        ...
        cache_state_of_hybrid(request_index,:,:)=cache_state;        
    end
    ...
    e2e_expected_delay_of_each_request_of_hybrid=zeros(number_of_request,1);
    ...
    packet_loss_of_each_request_of_hybrid=zeros(number_of_request,3);
    ...
    packet_loss_of_each_request_of_hybrid_of_hop_level=zeros(number_of_node,3);
    ...
    user_received_size_of_hybrid=zeros(number_of_request,1);
    ...
    for step = 1:num_steps
        ...
        left_time_of_hybrid=time_step*ones(number_of_request,1);
        ...
        %路由按照G_with_propogation_delay生成的，去计算各个低轨卫星传各个视频的packet需要多长时间、各个低轨卫星的丢包率
        %e2e_expected_delay_of_packet_of_comparison_fake和real分别指的是不考虑丢包的时延和考虑丢包的时延，e2e_expected_delay_and_packet_loss_of_packet函数的最后一个变量分别赋的2和1
        [e2e_expected_delay_of_packet_of_hybrid_fake,packet_loss_of_packet_of_hybrid,number_of_hop] = e2e_expected_delay_and_packet_loss_of_packet(number_of_node,number_of_video,cache_state,T,G_with_packet_loss,G_with_propogation_delay,G_with_packet_loss,G_with_relay_process_delay,maximum_times_of_ARQ,FEC_parameter,bandwidth_of_transmitter,packet_size,2);
        [e2e_expected_delay_of_packet_of_hybrid_real,packet_loss_of_packet_of_hybrid,number_of_hop] = e2e_expected_delay_and_packet_loss_of_packet(number_of_node,number_of_video,cache_state,T,G_with_packet_loss,G_with_propogation_delay,G_with_packet_loss,G_with_relay_process_delay,maximum_times_of_ARQ,FEC_parameter,bandwidth_of_transmitter,packet_size,1);
        %*****************************Stop:根据缓存状态、网络拓扑图图的边属性生成各node在packet level的delay和packet loss*************************************

        %*****************************Begin：测试hybrid方法（即选择node按距离优先，路由路径按packet优先）的性能*****************************
        %**********计算传各个视频需要多长时间、传各个视频会丢百分之多少的包**********
        for request_index=1:1:number_of_request
            [sorted_result,sorted_node_index]=sort(e2e_expected_delay_of_packet_of_hybrid_fake(:,video_request(request_index)),'ascend');%注意这里用的是e2e_expected_delay_of_packet_of_hybrid_fake
            for node_index=1:1:number_of_node
                %首先判断node是否缓存了video，没有的话直接换下一个node
                if isnan(e2e_expected_delay_of_packet_of_hybrid_real(sorted_node_index(node_index),video_request(request_index)))
                    continue;
                end
                
                %首先判断缓存的内容是否足以恢复视频
                if user_received_size_of_hybrid(request_index)+cache_state_of_hybrid(request_index,sorted_node_index(node_index),video_request(request_index))<size_of_video
                    deliver_time=min(left_time_of_hybrid(request_index),cache_state_of_hybrid(request_index,sorted_node_index(node_index),video_request(request_index))/packet_size*e2e_expected_delay_of_packet_of_hybrid_real(sorted_node_index(node_index),video_request(request_index)));
                else
                    deliver_time=min(left_time_of_comparison(request_index),(size_of_video-user_received_size_of_comparison(request_index))/packet_size*e2e_expected_delay_of_packet_of_comparison_real(sorted_node_index(node_index),video_request(request_index)));
                end
                
                %计算传了多少包，丢了多少包。min函数是考虑到剩余时间截止时，有些包可能传不完
                number_of_delivered_packet=deliver_time/e2e_expected_delay_of_packet_of_hybrid_real(sorted_node_index(node_index),video_request(request_index));
                number_of_effective_delivered_packet=number_of_delivered_packet*(1-packet_loss_of_packet_of_hybrid(sorted_node_index(node_index)));
                number_of_loss_packet=number_of_delivered_packet*packet_loss_of_packet_of_hybrid(sorted_node_index(node_index));
                
                %对缓存中未发的视频大小、用户收到的视频大小、剩余时间进行更新
                cache_state_of_hybrid(request_index,sorted_node_index(node_index),video_request(request_index))=cache_state_of_hybrid(request_index,sorted_node_index(node_index),video_request(request_index))-number_of_effective_delivered_packet*packet_size;                
                user_received_size_of_hybrid(request_index)=user_received_size_of_hybrid(request_index)+number_of_effective_delivered_packet*packet_size;
                left_time_of_hybrid(request_index)=left_time_of_hybrid(request_index)-deliver_time;                
                
                %进行KPI的记录
                e2e_expected_delay_of_each_request_of_hybrid(request_index)=e2e_expected_delay_of_each_request_of_hybrid(request_index)+deliver_time;                
                packet_loss_of_each_request_of_hybrid(request_index,1)=packet_loss_of_each_request_of_hybrid(request_index,1)+number_of_loss_packet;
                packet_loss_of_each_request_of_hybrid(request_index,2)=packet_loss_of_each_request_of_hybrid(request_index,2)+number_of_delivered_packet;
                packet_loss_of_each_request_of_hybrid_of_hop_level(number_of_hop(sorted_node_index(node_index)),1)=packet_loss_of_each_request_of_hybrid_of_hop_level(number_of_hop(sorted_node_index(node_index)),1)+number_of_loss_packet;
                packet_loss_of_each_request_of_hybrid_of_hop_level(number_of_hop(sorted_node_index(node_index)),2)=packet_loss_of_each_request_of_hybrid_of_hop_level(number_of_hop(sorted_node_index(node_index)),2)+number_of_delivered_packet;

                %如果视频video_index的传输时长time_step耗尽，则结束对视频video_index的传输
                if left_time_of_hybrid(request_index)<0.001
                    left_time_of_hybrid(request_index)=0;
                    break;
                end
                
                %如果用户已经收到了完整视频video_index，则结束对视频video_index的传输
                if user_received_size_of_hybrid(request_index)>=size_of_video
                    break;
                end
            end
        end
    ...
    average_delay_of_hybrid=sum(e2e_expected_delay_of_each_request_of_hybrid)/number_of_request;
    %*****************************Stop:时延结果统计*************************************
    ...
    for request_index=1:1:number_of_request
        ...
        packet_loss_of_each_request_of_hybrid(request_index,3)=packet_loss_of_each_request_of_hybrid(request_index,1)/packet_loss_of_each_request_of_hybrid(request_index,2);
    end
    ...
    average_packet_loss_of_request_of_hybrid=sum(packet_loss_of_each_request_of_hybrid(:,3))/number_of_request;    
    ...
    for hop_index=1:1:number_of_node
        ...
        packet_loss_of_each_request_of_hybrid_of_hop_level(hop_index,3)=packet_loss_of_each_request_of_hybrid_of_hop_level(hop_index,1)/packet_loss_of_each_request_of_hybrid_of_hop_level(hop_index,2);
    end   
    %*****************************Stop:丢包率结果统计*************************************
    ...
    monte_carlo_average_delay_of_hybrid(monte_carlo_index)=average_delay_of_hybrid;
    ...
    monte_carlo_average_packet_loss_of_hybrid(monte_carlo_index)=average_packet_loss_of_request_of_hybrid;
    ...
    for hop_index=1:1:number_of_node
        ...
        monte_carlo_average_packet_loss_of_hybrid_of_hop_level(hop_index,monte_carlo_index)=packet_loss_of_each_request_of_hybrid_of_hop_level(hop_index,3);
    end   
...
average_monte_carlo_average_delay_of_hybrid=sum(monte_carlo_average_delay_of_hybrid)/monte_carlo_iteration;
...
average_monte_carlo_average_packet_loss_of_hybrid=sum(monte_carlo_average_packet_loss_of_hybrid)/monte_carlo_iteration;
...
average_monte_carlo_average_packet_loss_of_hybrid_of_hop_level=zeros(number_of_node,1);
...
for hop_index=1:1:number_of_node
    ...
    average_monte_carlo_average_packet_loss_of_hybrid_of_hop_level(hop_index)=nanmean(monte_carlo_average_packet_loss_of_hybrid_of_hop_level(hop_index,:));
end
%*****************************Stop:计算时延、视频丢包率、各跳丢包率的蒙特卡洛均值结束*************************************

% %画hop level的packet loss图
% for i=1:1:min([sum(~isnan(average_monte_carlo_average_packet_loss_of_proposed_of_hop_level)),sum(~isnan(average_monte_carlo_average_packet_loss_of_comparison_of_hop_level)),sum(~isnan(average_monte_carlo_average_packet_loss_of_random_dis_of_hop_level)),sum(~isnan(average_monte_carlo_average_packet_loss_of_random_pac_of_hop_level))])%该函数用于统计4个数组中非nan元素的最小值
%    hop_index(i)=i;
% end
% 
% KPI_x=[4,6];
...
% 
% figure(1)
% plot(hop_index,average_monte_carlo_average_packet_loss_of_proposed_of_hop_level(1:max(hop_index)),'-mp','LineWidth',2.5,'MarkerSize',14);
% hold on
% plot(hop_index,average_monte_carlo_average_packet_loss_of_comparison_of_hop_level(1:max(hop_index)),'-go','LineWidth',2.5,'MarkerSize',14);
% hold on
% plot(hop_index,average_monte_carlo_average_packet_loss_of_random_dis_of_hop_level(1:max(hop_index)),'-b+','LineWidth',2.5,'MarkerSize',14);
% hold on
% plot(hop_index,average_monte_carlo_average_packet_loss_of_random_pac_of_hop_level(1:max(hop_index)),'-c*','LineWidth',2.5,'MarkerSize',14);
% hold on
% plot(KPI_x,KPI_y,'-rd','LineWidth',2.5,'MarkerSize',14);
% 
% grid on
% set(gca,'FontSize',35,'fontname','Times New Roman','linewidth',1.5); 
% xlabel('Hops','fontname','Times New Roman','FontSize',35);
% xlim([1 max(hop_index)]);
% ylabel('Packet loss Rate','fontname','Times New Roman','FontSize',35);
% %legend('Proposed','Distance Aware','Random Type-1','Random Type-2','Location','northwest','FontSize',30,'fontname','Times New Roman');
% legend('Proposed','Distance Aware','Random Type-I','Random Type-II','KPI','Location','northwest','FontSize',30,'fontname','Times New Roman');
